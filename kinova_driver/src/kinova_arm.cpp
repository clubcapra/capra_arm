  ...
  , kinova_robotName_(kinova_robotName)
{
  ...
  ovis_home_service_ =
      node_handle_.advertiseService("in/home_joint_positions", &KinovaArm::ovisHomePositionSrvCallback, this);
  ...
  ovis_joint_position_publisher_ = node_handle_.advertise<ovis_msgs::OvisJointPosition>("out/joint_position", 2);
  ...
  cartesian_velocity_with_finger_velocity_subscriber_ = node_handle_.subscribe(
      "in/cartesian_velocity_with_finger_velocity", 1, &KinovaArm::cartesianVelocityWithFingerVelocityCallback, this);
  ...
  cartesian_trajectory_point.InitStruct();
  cartesian_trajectory_point.Position.Type=ANGULAR_POSITION;
  cartesian_trajectory_point.Position.Delay=0.0;
...
void KinovaArm::ovisCartesianPositionCallback(const ovis_msgs::OvisJointPosition::ConstPtr& msg)
{
  this->cartesian_trajectory_point.Position.Actuators.Actuator1 = msg->joint_positions[0];
  this->cartesian_trajectory_point.Position.Actuators.Actuator2 = msg->joint_positions[1];
  this->cartesian_trajectory_point.Position.Actuators.Actuator3 = msg->joint_positions[2];
  this->cartesian_trajectory_point.Position.Actuators.Actuator4 = msg->joint_positions[3];
  this->cartesian_trajectory_point.Position.Actuators.Actuator5 = msg->joint_positions[4];
  this->cartesian_trajectory_point.Position.Actuators.Actuator6 = msg->joint_positions[5];
...
void KinovaArm::cartesianVelocityWithFingerVelocityCallback(
    const kinova_msgs::PoseVelocityWithFingerVelocityConstPtr& cartesian_vel_with_finger_velocity)
{
  if (!kinova_comm_.isStopped())
  {
...
void KinovaArm::publishJointAngles(void)
{
  ...
  joint_state.position[0] = kinova_angles.joint1 * M_PI / 180 - M_PI;
  joint_state.position[1] = -kinova_angles.joint2 * M_PI / 180 - M_PI;
  joint_state.position[2] = -kinova_angles.joint3 * M_PI / 180 - M_PI;
  joint_state.position[3] = -kinova_angles.joint4 * M_PI / 180 - M_PI;
  if (arm_joint_number_ >= 6)
  {
    joint_state.position[4] = kinova_angles.joint5 * M_PI / 180 - M_PI;
    joint_state.position[5] = kinova_angles.joint6 * M_PI / 180 - M_PI;
  }
  if (arm_joint_number_ == 7)
  {
    joint_state.position[6] = kinova_angles.joint7 * M_PI / 180 - M_PI;
  }
...
void KinovaArm::ovisPublishJointPosition(void)
{
  KinovaAngles current_angles;
  kinova_comm_.getJointAngles(current_angles);
  ovis_msgs::OvisJointPosition ovis_joint_position_msg;
  ovis_joint_position_msg.joint_positions[0] = current_angles.Actuator1;
  ovis_joint_position_msg.joint_positions[1] = current_angles.Actuator2;
  ovis_joint_position_msg.joint_positions[2] = current_angles.Actuator3;
